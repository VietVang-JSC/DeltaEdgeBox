...
<html lang="en">
  <head>
    ...
    <style>
        ...
        .store-detail {
            display: block; /* Đảm bảo mỗi thông tin là một dòng riêng biệt */
            margin: 2px 0; /* Tạo khoảng cách giữa các dòng */
        }
    ...
    <div class="invoice mt-0" id="print">
            ...
            <div class="invoice-info">
                ...
                <table class="info-table" aria-label="Thông tin hóa đơn">
                    <tr>
                        ...
                        <td class="Info_staff_input current_date">{{ $payment['updated_at'] ?? $payment['created_at'] ?? now($timeZone ?? config('app.timezone'))->format('Y-m-d H:i:s') }}</td>
                    </tr>
                    <tr>
       ...
       <div class="invoice-total p-2">
            @if (!empty($qrImagePath))
                <img src="{{ $qrImagePath }}" style="height:360px;" alt="QR Code"> <br>
            @endif
